...
export default function AddLandmarkModal({
  ...
  const { user } = useUser();
  const addLandmarkMutation = trpc.landmarks.add.useMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const userAvatar = user.profile?.profilePictureUrl;
      const validImages = images.filter(img => img.startsWith('http://') || img.startsWith('https://'));
      
      const result = await addLandmarkMutation.mutateAsync({
        name: locationName,
        type: selectedType,
        coordinates: coordinates,
        userNote: note.trim() || undefined,
        userImages: validImages.length > 0 ? validImages : undefined,
        userId: user.id,
        userName: userName,
        userAvatar: userAvatar && (userAvatar.startsWith('http://') || userAvatar.startsWith('https://')) ? userAvatar : undefined,
      });
      
      console.log("[AddLandmark] Landmark saved to backend:", result.landmark);
      onAdd(result.landmark);
      resetForm();
      onClose();
      ...
      if (error.message) {
        if (error.message.includes("404") || error.message.includes("not found")) {
          errorMessage = "Server is starting up. Please try again in a few seconds.";
        } else if (error.message.includes("JSON Parse")) {
          errorMessage = "Server is restarting. Please wait 10 seconds and try again.";
        } else {
            ...
            <View style={styles.formContainer}>
              ...
              >
                ...
                <Text style={styles.imageButtonText}>
                  {images.length > 0 ? `${images.length} photo(s) added` : "Add Photos"}
                </Text>
              </TouchableOpacity>
